...
let APP_SECRET = process.env.ACCESS_TOKEN;

let ops = commandLineArgs([
    {name: 'lt', alias: 'l', args: 1, description: 'Utilise localtunnel.me pour mettre le bot sur internet', type: Boolean, defaultValue: false},
    {name: 'ltsubdomain', alias: 's', args: 1, description: 'Sous domaine pour localtunnel.me. Ne peut être utilisé que avec --lt', type:String, defaultValue:null}]);

if (ops.lt === false && ops.ltsubdomain !== null) {
    console.log("error: --ltsubdomain ne peut être que utilisé avec --lt");
    process.exit();
}
...
controller.setupWebserver(PORT, function(err, webserver) {
    ...
    webserver.get('/cgu',function(req,res) {
        res.sendFile('/views/cgu.html', {root: __dirname});
    });

    controller.createWebhookEndpoints(webserver, bot, function() {
        console.log('ONLINE!');
        if(ops.lt) {
            let tunnel = localtunnel(PORT, {subdomain: ops.ltsubdomain}, (err, tunnel) => {
                if(err) {
                    console.log(err);
                    process.exit();
                }
                console.log("Le bot est online sur " + tunnel.url);
            });
    ...
    if(onlineBots[bot.config.token]) {
        ...
        bot.startRTM((err) => {
          ...
          if(err) {
            ...
            return setTimeout(startRTM, 6000);
          } else {
              addBot(bot)
          }
        });
...
let evenements = 'message_received';

function hearsDefaultEvenement(motsClefs, reponses) {
    controller.hears(motsClefs, evenements, (bot, message) => {
        if(message) {
...
function hearsDefaultEvenementWithCallback(motsClefs, callback) {
    controller.hears(motsClefs, evenements, callback);
}
...
function defis(nomDefis, reponseDefis, reponseIndice) {
    controller.hears(['(.*)(' + nomDefis + ')(.*)'], evenements, (bot, message) => {
        ...
        // Défis peut être a gauche du nom du défis
        if(partieGauche) {
...
hearsDefaultEvenement(['🍆'], ["Ah, je pense que tu as besoin de contacter notre star académique !"]);
...
hearsDefaultEvenement(['(Ulysse|Ulysse|ulysse|ulyse) (aux|au) (mille|milles) (expédients|expedient|expedients)'], ["Je vois que tu as utilisé le nom de code et le mot de passe. Tu dois mettre sur l'event facebook le message suivant si tu veux gagner le défi secret : Petit oiseau si tu n\'as pas d\'aile tu peux pas voler."]);

controller.hears(['^ah$', 'ah !'], evenements, (bot, message) => {
        let reponses = ["HA !"];
        let image = {
            "attachment": {
                "type":"audio",
                "payload": {
                    "url": "https://github.com/TiagoCerqueira/filesForBotSlack/raw/master/audioclip-1491911507000-1071.mp3"
                }
            }
        };
    ...
    controller.hears(['Rennes', 'rennes'], evenements, (bot, message) => {
        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://www.santatelevision.com/perenoel/files/image_renne_laponie_finlande.jpg"
                }
            }
        };
    ...
    controller.hears(['comptable'], evenements, (bot, message) => {
        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "https://scontent-cdg2-1.xx.fbcdn.net/v/t31.0-8/s960x960/17966985_10211287933905536_8865472063882881888_o.jpg?oh=2f0884dbe5accc255c5cd2401917bbc2&oe=59861E87"
                }
            }
        };
    ...
    controller.hears(['au revoir', 'Au revoir'], evenements, (bot, message) => {
        let reponses = ["C'est ça ! Vas y, casse toi !"];
        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "https://media.giphy.com/media/hmxZRW8mhs4ak/giphy.gif"
                }
            }
        };
    ...
    controller.hears(['cerbère','cerbere'], evenements, (bot, message) => {
        let reponses = ["Le cerbère est le gardien de la porte des enfers. C'est une espèce de gros chien à trois têtes."];
        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://www.dinosoria.com/religion/cerbere.jpg"
                }
            }
        };
    ...
    controller.hears(['morse'], evenements, (bot, message) => {
        let reponses = ["Tu as besoin de décoder du morse ? Tiens ! "];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://md5decrypt.net/Code-morse/morse.gif"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)9', 'indice(.*)9'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 9 🗡️"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/4732311805237913002149000752541985018620o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)1', 'indice(.*)1'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 1 ⚡"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/6802901804972113002153634085412075446581o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)3', 'indice(.*)3'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 3 💜"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/3759021805311613002152134085561399534913o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)2', 'indice(.*)2'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 2 🔰"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/2315931804988213002151900752251762907539o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)5', 'indice(.*)5'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 5 🏠"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/2651891804946113002153267418781798486674o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)\d{1}0', 'indice(.*)\d{1}0'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 10 💀"];
    ...
    controller.hears(['Indice(.*)7', 'indice(.*)7'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 7 🎼"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/7830221805322013002148567419251541643679o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)8', 'indice(.*)8'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 8 ⏳"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/200238180497601300215023408575276165453o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)4', 'indice(.*)4'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 4 💌"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/5128671801588313002152167418891497810311o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)\d{1}1', 'indice(.*)\d{1}1'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 11 🌌"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/295015180524771300215336741877561053177o.jpg"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)6', 'indice(.*)6'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 6 🍷"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/206706DefiDionysos.png"
                }
            }
        };
    ...
    controller.hears(['Indice(.*)\d{1}2', 'indice(.*)\d{1}2'], evenements, (bot, message) => {
        let reponses = ["Voici un indice pour le défi 12 🌄"];

        let image = {
            "attachment": {
                "type":"image",
                "payload": {
                    "url": "http://img15.hostingpics.net/pics/6423171805248113002150134085761531080646o.jpg"
                }
            }
        };
...
defis('Athéna|athéna|Athena|athena', "Pour le défi d\'Athéna l\'équipe doit répondre juste à ce test. https://goo.gl/forms/WEHlaE65WwgCFQ0L2 Je rappelle que je suis un membre à part entière de l\'organisation. ;) ", "Indice pour le défi Athéna : ");
...
defis('Hermès|Hermes|hermès|hermes', "Pour le défi Hermès, dieux du voyage, de la communication, des commerces et de la diplomatie vous allez devoir trouver la sainte bière dans l'établissement au Velvet.");
...
hearsDefaultEvenement(['(GT|Groupe de travail) (Event|event|évent|événement|Événement)'], ["Le questionnaire du groupe de travail événement, c'est ici : https://goo.gl/forms/2geeGE9npZ9ACij52 😉"]);
hearsDefaultEvenement(['(GT|Groupe de travail) (Girlz|girlz|femmes|Femmes|femme|Femme) (in|en) MIAGE'], ["Le questionnaire du  groupe de travail Girlz in MIAGE, c'est ici : https://goo.gl/forms/2ZtFHFu33tapTgHa2 😉"]);
...
hearsDefaultEvenement(['(Formation|formation|forma|Forma) (alumni|Alumni|Papi|papi|vieux)'], ["Le questionnaire de la formation Alumni, c'est ici : https://goo.gl/forms/P0kQaWIk0r94y6Fm2 😉"]);
